...
class DatabaseService:
    ...
    def __init__(self):
        ...
        logger.info(f"Database service initialized: {settings.db_host}:{settings.db_port}/{settings.db_name}")
    ...
    async def save_document_metadata(
        ...
        try:
            async with self.get_session() as session:
                ...
                await session.execute(document_query, {
                    'id': doc_uuid,
                ...
                await session.commit()
                logger.info(f"Saved document metadata for {document_id} with UUID {doc_uuid}")
                return doc_uuid
    ...
    async def save_document(self, document_data: Dict[str, Any]) -> str:
        ...
        try:
            async with self.get_session() as session:
                ...
                doc_query = text("""
                    INSERT INTO documents (
                        document_id, filename, file_size, document_type, 
                        summary, confidence_score, pages_processed, 
                        chunks_created, vectors_stored, processing_time_seconds,
                        processing_status
                    ) VALUES (
                        :document_id, :filename, :file_size, :document_type,
                        :summary, :confidence_score, :pages_processed,
                ...
                result = await session.execute(doc_query, {
                    ...
                    'processing_time_seconds': document_data.get('processing_time_seconds'),
        ...
        try:
            async with self.get_session() as session:
                for i, topic in enumerate(topics):
                    ...
                    await session.execute(topic_query, {
    ...
    async def save_document_entities(self, doc_uuid: str, entities: List[Dict[str, Any]]) -> None:
        ...
        try:
            async with self.get_session() as session:
                for entity in entities:
                    ...
                    await session.execute(entity_query, {
                        'document_id': doc_uuid,
    ...
    async def save_document_chunks(self, doc_uuid: str, chunks_data: List[Dict[str, Any]]) -> None:
    ...
    async def get_document_by_id(self, document_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            async with self.get_session() as session:
                query = text("""
                    SELECT d.*, 
                           json_agg(DISTINCT dt.topic) as topics,
                           json_agg(DISTINCT de.entity_text) as entities
                    FROM documents d
    ...
    async def log_search_query(self, query_text: str, results_count: int, 
                              response_time_ms: int, user_session: str = None) -> None:
        """Log search query for analytics."""
        try:
            async with self.get_session() as session:
                search_query = text("""
                    INSERT INTO search_queries (
                        query_text, results_count, response_time_ms, user_session
                    ) VALUES (
                        :query_text, :results_count, :response_time_ms, :user_session
                    )
                """)
                
                await session.execute(search_query, {
    ...
    async def log_chat_conversation(self, session_id: str, user_message: str, 
                                   ai_response: str, sources_used: List[str],
            ...
            async with self.get_session() as session:
                chat_query = text("""
                    INSERT INTO chat_conversations (
                        session_id, user_message, ai_response, sources_used, response_time_ms
                    ) VALUES (
                        :session_id, :user_message, :ai_response, :sources_used, :response_time_ms
                    )
                """)
                
                await session.execute(chat_query, {
    ...
    async def save_vbc_contract(self, doc_uuid: str, vbc_data: 'VBCContractData') -> str:
            ...
            async with self.get_session() as session:
                ...
                contract_query = text("""
                    INSERT INTO vbc_contracts (
                        document_id, agreement_id, agreement_title, country, disease_area, 
                        disease_area_details, patient_population_size, patient_population_description,
                        agreement_overview, contract_background, pilot_program_results,
                        duration_months, duration_description, data_collection_frequency,
                        regulatory_framework, governance_structure, contract_complexity,
                ...
                contract_result = await session.execute(contract_query, {
                    ...
                    'contract_complexity': vbc_data.contract_complexity.value if vbc_data.contract_complexity else None,
                ...
                await session.commit()
                logger.info(f"Successfully saved complete VBC contract data for ID: {contract_id}")
    ...
    async def _save_vbc_parties(self, session, contract_id: str, vbc_data: 'VBCContractData'):
        """Save contract parties."""
        if vbc_data.parties:
            for party in vbc_data.parties:
                party_query = text("""
                    INSERT INTO vbc_contract_parties (
                        contract_id, party_name, party_type, party_country
                    ) VALUES (:contract_id, :party_name, :party_type, :party_country)
                """)
                
                await session.execute(party_query, {
    ...
    async def _save_vbc_financials(self, session, contract_id: str, vbc_data: 'VBCContractData'):
        """Save financial structure."""
        if vbc_data.financial_structure:
            financial_query = text("""
                INSERT INTO vbc_contract_financials (
                    contract_id, initial_payment, currency, payment_model,
                    base_reimbursement, shared_savings_percentage,
                    risk_corridor_upper, risk_corridor_lower, 
                    maximum_payout, minimum_guarantee
                ) VALUES (
                    :contract_id, :initial_payment, :currency, :payment_model,
                    :base_reimbursement, :shared_savings_percentage,
                    :risk_corridor_upper, :risk_corridor_lower,
            ...
            await session.execute(financial_query, {
                'contract_id': contract_id,
    ...
    async def _save_vbc_risk_protection(self, session, contract_id: str, vbc_data: 'VBCContractData'):
        """Save risk protection mechanisms."""
        if vbc_data.risk_protection:
            risk_query = text("""
                INSERT INTO vbc_contract_risk_protection (
                    contract_id, has_stop_loss, has_risk_cap, has_non_responder_fund,
                    stop_loss_threshold, risk_cap_percentage
                ) VALUES (
                    :contract_id, :has_stop_loss, :has_risk_cap, :has_non_responder_fund,
                    :stop_loss_threshold, :risk_cap_percentage
                )
    ...
    async def _save_vbc_metrics(self, session, contract_id: str, vbc_data: 'VBCContractData'):
        ...
        if vbc_data.outcome_metrics:
            for metric in vbc_data.outcome_metrics:
                ...
                await session.execute(metric_query, {
                    'contract_id': contract_id,
    ...
    async def _save_vbc_endpoints(self, session, contract_id: str, vbc_data: 'VBCContractData'):
        ...
        if vbc_data.primary_endpoints:
            for endpoint in vbc_data.primary_endpoints:
                ...
                await session.execute(endpoint_query, {
                    'contract_id': contract_id,
